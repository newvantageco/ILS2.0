...
(async () => {
  ...
  await client.query(`
    ...
    ON CONFLICT (email) DO UPDATE SET role = 'platform_admin', is_active = true
  `);

  const result = await client.query(`SELECT email, role, is_active FROM users WHERE email = 'care@newvantageco.com'`);
  console.log('✅ Platform Admin Created!');
  console.log(result.rows[0]);
  console.log('\nLogin: care@newvantageco.com / Eyecloud123');
